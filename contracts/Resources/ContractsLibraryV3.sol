...
import "./IContractsLibraryV3.sol";
import "./IUniswapV2Pair.sol";

contract ContractsLibraryV3 is AccessControl, IContractsLibraryV3 {
    ...
    function getInForTokenToBnbToAltToken(
        address token,
        address altToken,
        uint _amount
    ) external view override returns (uint256) {        
        ...
        _addressArray[1] = WBNB();
        _addressArray[2] = altToken;
        uint[] memory _amounts = ROUTER().getAmountsIn(_amount, _addressArray);
        return _amounts[0];
    }
